...
Document.prototype = {
	...
	convertTextToCitations: async function () {
		Zotero.debug("ZoteroWinWordIntegration: convertTextToCitations", 4);
		checkIfFreed(this._documentStatus);

		// Prompt user for Regex
		// We use basic prompt service.
		var { Services } = ChromeUtils.import("resource://gre/modules/Services.jsm");
		var promptService = Services.prompt;

		var regexStr = { value: "\\[(.*?)\\]\\(zotero://select/items/([A-Z0-9]{8})\\)" }; // Default example
		var check = { value: false };
		var result = promptService.prompt(null, "Convert Text Citations", "Enter Regex Pattern (Group 1 must be the Key):", regexStr, null, check);

		if (!result) return;

		var groupIndex = 1; // Hardcoded for now, or ask user?
		// We could use a more complex dialog, but prompt is simple.
		// Let's assume Group 1.

		var fieldListNode = new fieldListNode_t.ptr();
		checkStatus(f.convertRegexToFields(this._document_t, regexStr.value, groupIndex, fieldListNode.address()));
		...
		for (let field of fields) {
			let itemKey = field.getCode();
			// We need to fetch the item. Assuming we are in Zotero environment where Zotero.Items is available.
			// However, this code runs in Zotero's process, so Zotero globals are available.
			// But we need to handle the case where item is not found or key format is weird.

			// Warning: itemKey might need trimming if C++ didn't handle null term correctly or something, 
			// but readString() should handle it.

			// We need to find the item. We assume the key belongs to one of the user's libraries.
			// A global search or search in active library is needed.
			// Zotero.Integration.formatCitation returns the text.

			// For simplicity, we assume 'style' is already set in document prefs or we prompt?
			// The user just wants conversion. We should respect doc prefs if they exist, or set them up?
			// If doc is new, it might not have prefs.

			// Let's assume the doc has comps/prefs or we trigger the 'Add Citation' flow logic?
			// No, we want to construct the field code directly.

			try {
				// We need to find the item.
				let item = await Zotero.Items.getByLibraryAndKeyAsync(1, itemKey); // Default to My Library (1) ?
				// Better: Search all libraries?
				if (!item) {
					// try to find in other libraries
					// This is complex. For now, try library 1.
					// If not found, log error and leave field as is (showing key).
					Zotero.debug("Item not found for key: " + itemKey);
					continue;
				}

				// Construct CSL Citation Data
				let citationData = {
					"id": item.id,
					"uris": [Zotero.URI.getItemURI(item)],
					"itemData": Zotero.Cite.getItemData(item)
				};

				// We need a full citation structure
				let cslCitation = {
				...
				// We need to format it. This requires a style.
				// If document has no style, we default to APA?
				// Accessing document prefs requires parsing "ZOTERO_PREF" property.
				// If not present, we can't format correctly.
				// Maybe we just set the CODE content to "ADDIN ZOTERO_ITEM ..." and let "Refresh" handle formatting?
				// Yes! That's safer.

				let fieldCode = "ADDIN ZOTERO_ITEM CSL_CITATION " + JSON.stringify(cslCitation);
				field.setCode(fieldCode);
				field.setText("Loading..."); // Placeholder
			} catch (e) {
				Zotero.debug("Error processing field: " + e);
			}
			await Zotero.Promise.delay(10);
		}

		// Trigger refresh to format citations
		// This requires integration to be initialized for this doc.
		// If we return, Zotero Integration might handle it?
		// We should call a Refresh command.
		// But we don't have access to Zotero Integration instance here easily?
		// Actually 'this' is the Document instance.
		// logic usually resides in `integration.js`.
		// We are in the lower level generic layer. 
		// We should probably just return the fields, or let the caller handle refresh.
		// But the requirement says "Call Zotero core ...".
